...
<div class="row g-4" id="toursContainer">
    ...
    <?php else: ?>
        <?php foreach ($tours as $tour): ?>
            ...
            $searchText = strtolower($tour['name'] . ' ' . $tour['location'] . ' ' . ($tour['country'] ?? ''));
            ?>
            <div class="col-md-6 col-lg-4 tour-card" data-search="<?= htmlspecialchars($searchText) ?>">
                <div class="card border-0 shadow-sm h-100 rounded-4 overflow-hidden">
                    <img src="<?= BASE_URL . ltrim($tour['image'], '/') ?>" class="card-img-top"
                        alt="<?= htmlspecialchars($tour['name']) ?>" style="height: 220px; object-fit: cover;">
                    <div class="card-body p-4">
                        <p class="small text-muted mb-1">
                            <i class="bi bi-person-circle me-1"></i><?= htmlspecialchars($tour['guide_name'] ?? 'Eco Guide') ?>
                        </p>
                        <h5 class="card-title fw-bold"><?= htmlspecialchars($tour['name']) ?></h5>
                            ...
                            <div class="mb-2">
                                <?php foreach ($impactTags as $tag): ?>
                                    <span class="badge bg-success bg-opacity-25 text-success me-1 mb-1">🌱
                                        <?= ucfirst(str_replace('_', ' ', $tag)) ?></span>
                                <?php endforeach; ?>
                            </div>
                        <?php endif; ?>
                        <div class="d-flex justify-content-between align-items-center mt-3">
                            <h5 class="fw-bold text-success mb-0">$<?= number_format($tour['price'] ?? 0) ?></h5>
                            <a href="<?= BASE_URL ?>guest/tour/<?= $tour['tour_id'] ?>"
                                class="btn btn-outline-success rounded-pill">Details</a>
                        </div>
                    </div>
                </div>
            </div>
